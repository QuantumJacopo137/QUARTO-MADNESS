...
integer :: wb_index=1
integer :: win_index, n_win, loss_index, n_loss
...
integer :: max_ahead=6 !index and max value of forward predicting
...
allocate(win_board_scores(holes,5))
win_board_scores=0
...
if (max_ahead.gt.holes-1) then
max_ahead = holes-1
endif

write(*,*) 'Max num of move ahead:', max_ahead
...
! if this is the case, skip to the next one
 ...
 ! now i add 1 to the unavailable pieces number, bc I'm trying to place the opponent piece on the board
 Unav = Unav + 1 
...
do x=1,samples
 ...
 do fi=1,max_ahead
  ...
  if (win.and.modulo(fi,2).eq.0.or.(win_var(1)+win_var(2).eq.2.or.win_var(3)+win_var(4).eq.2.or.win_var(5)+win_var(6).eq.2.or.win_var(7)+win_var(8).eq.2.and.modulo(fi,2).eq.1)) then
  ...
  else if (win.and.modulo(fi,2).eq.1.or.(win_var(1)+win_var(2).eq.2.or.win_var(3)+win_var(4).eq.2.or.win_var(5)+win_var(6).eq.2.or.win_var(7)+win_var(8).eq.2.and.modulo(fi,2).eq.0)) then
...
write(*,*) '$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$'
write(*,*) ''
104 format(F3.0,3X,F3.0,5X, F10.5, 3X, F10.5, 5X, F10.5)

write(*,*) 'WIN SCORE [ i ; j ; avrg win moves ; avrg loss moves ; probability of winning ]:'
write(*,*) ''
do i=1,holes-1
write(*,104) win_board_scores(i,:)
enddo

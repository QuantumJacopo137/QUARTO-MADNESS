...
! THIS PROGRAM WILL SCAN THE BEST MOVE FOR YOU TO GET THE WIN BY MONTE CARLO SAMPLING


! FORMATTING AND STARTING THE GAME: TELL THE PROGRAM WHAT'S THE SITUATION LIKE
write(*,*) 'How many pieces are there on the board?'
read(*,*) n_pices_i
write(*,*) 'Now, the first piece placed will be (+1,+1,+1,+1), and will define the legend, the variables can be +1 or -1'
write(*,*) ''
write(*,*) 'ORDER OF PROPRIETIES:'
write(*,*) '1st - Color'
write(*,*) '2nd - Hight'
write(*,*) '3rd - Shape'
write(*,*) '4th - Filling'
write(*,*) ''
write(*,*) 'Choose a set of axis starting from the corner. Indices will go from 1 to 4'
...
! CREATE THE BOARD
! THIS TIME WILL BE +1 OR -1 FOR EACH PROPRIETY AND 0 FOR HOLES
...
! Create the array that will accumulate the score for each moove


allocate(win_board_scores(16-n_pices_i,3))
win_board_scores=0
...
do k=1,n_pices_i
 ...
 write(*,*) 'Piece number ', k, 'Has been placed in:'
 write(*,fmt='(A)', advance="no") 'i='
 read(*,*) ii
 write(*,fmt='(A)', advance="no") 'j='
 read(*,*) jj
! STORES THE OCCUPIED POSITION IN THIS ARRAY
 Unavaliable_pos(Unav,1)=ii 
 Unavaliable_pos(Unav,2)=jj
 

 if (k.eq.1) then
  bp(ii,jj,:)=[+1,+1,+1,+1] ! DEFINES THE FIRST PIECE AS THE 1111
 ! REMEMBERS THE USER ABOUT THE REFERENCE PROPERTIES
  write(*,*) 'Properties were:'

  write(unit=*,fmt='(A)', advance="no") 'Color, (Black or White): '
  read(*,*) Prop(1)

  write(*,fmt='(A)', advance="no") 'Height (Short o Tall): '
  read(*,*) Prop(2)

  write(*,fmt='(A)', advance="no") 'Shape (Circle or Square): '
  read(*,*) Prop(3)

  write(*,fmt='(A)', advance="no") 'Filling (Hollow or Solid): '
  read(*,*) Prop(4)
  Unavaliable_pieces(Unav,:)=1 


 else 
  write(*,*) 'Type the properties, relative to the first piece (+1 if the same -1 if opposite)'
  write(*,*) ''
  write(*,*) 'REMINDER: PIECE NUMBER ONE WAS:'
  do l=1,4
   write(*,*) Prop(l)
  enddo  
  write(*,*) ''
  write(*,*) 'Type +1/-1:'
  write(*,102, advance="no") 'Color:'
  read(*,*) bp(ii,jj,1)
  write(*,102, advance="no") 'Hight:'
  read(*,*) bp(ii,jj,2)
  write(*,102, advance="no") 'Shape:'
  read(*,*) bp(ii,jj,3)
  write(*,102, advance="no") 'Filling:'
  read(*,*) bp(ii,jj,4)

  Unavaliable_pieces(Unav,:)=bp(ii,jj,:)
!write(*,*) 'DEBUG 4'
  call UPDATE_AVAIL(avail_piec,bp(ii,jj,1),bp(ii,jj,2),bp(ii,jj,3),bp(ii,jj,4))

 endif
! counter to allocate mamory in arrays
Unav=Unav+1
enddo
...
write(*,*) 'Type the properties, relative to the first piece (+1 if the same -1 if opposite)'
  ...
  write(*,*) 'Type +1/-1:'
  write(*,102, advance="no") 'Color:'
  read(*,*) Opponent_choice(1)
  write(*,102, advance="no") 'Hight:'
  read(*,*) Opponent_choice(2)
  write(*,102, advance="no") 'Shape:'
  read(*,*) Opponent_choice(3)
  write(*,102, advance="no") 'Filling:'
  read(*,*) Opponent_choice(4)
